...
def main():
    ...
    # 60x68 transpose
    print("Running ./test-trans -M 60 -N 68")
    p = subprocess.Popen("./test-trans -M 60 -N 68 | grep TEST_TRANS_RESULTS", 
                         shell=True, stdout=subprocess.PIPE)
    stdout_data = p.communicate()[0].decode()
    ...
    print("%22s%8.1f%10d" % ("Total points", total_score,
                             ...
                             maxscore['trans64'] +
                             maxscore['trans60']))
    final_score = min(total_score / 60.0 * 100.0, 88)
    print("%22s%8.1f%10d" % ("Final points", final_score, 88))
    # Emit autoresult string for Autolab if called with -A option
    if autograde:
        autoresult="{\"scores\": {\"Autograded\": %.1f}, \"scoreboard\": [%.1f, %d, %d, %d, %d]}" % (total_score, final_score, miss32+miss64+miss60, miss32, miss64, miss60)
        print("%s" % autoresult)
