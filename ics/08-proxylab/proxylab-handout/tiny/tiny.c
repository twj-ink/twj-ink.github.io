...
int main(int argc, char **argv) 
{
    ...
    int getnameinfo_flags = NI_NUMERICHOST | NI_NUMERICSERV; // 强制返回数字地址和端口
    if (getnameinfo((SA *) &clientaddr, clientlen, hostname, MAXLINE, 
                    port, MAXLINE, getnameinfo_flags) != 0) {
...
{
    int srcfd;
    char *srcp, filetype[MAXLINE], resp[MAXBUF], buf[MAXLINE];
 
    /* Send response headers to client */
    get_filetype(filename, filetype);       //line:netp:servestatic:getfiletype
    strcpy(resp, "HTTP/1.0 200 OK\r\n");
    strcat(resp, "Server: Tiny Web Server\r\n");
    strcat(resp, "Connection: close\r\n");
    sprintf(buf, "Content-length: %d\r\n", filesize);
    strcat(resp, buf);
    strcat(resp, "Vary: *\r\n");
    strcat(resp, "Cache-Control: no-cache, no-store, must-revalidate\r\n");
    sprintf(buf, "Content-type: %.1024s\r\n\r\n", filetype);
    strcat(resp, buf);
    rio_writen(fd, resp, strlen(resp));       //line:netp:servestatic:endserve
...
void serve_dynamic(int fd, char *filename, char *cgiargs, char *headers) 
{
    char resp[MAXBUF], *emptylist[] = { NULL };
