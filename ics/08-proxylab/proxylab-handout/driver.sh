...
tiny_pid=$!
cd ${HOME_DIR}
echo "Setup done, running webdriver"

if [ -f ./webdriver_score--DO-NOT-FAKE-THIS-OR-YOU-WILL-GET-SCORE-DEDUCTION.txt ]; then
    rm -f ./webdriver_score--DO-NOT-FAKE-THIS-OR-YOU-WILL-GET-SCORE-DEDUCTION.txt
    if [ "$?" != "0" ]; then
        echo "failed to write webdriver score"
        exit
    fi
fi
echo 0 > webdriver_score--DO-NOT-FAKE-THIS-OR-YOU-WILL-GET-SCORE-DEDUCTION.txt
python3 -u ./webdriver_test.py ${proxy_port} ${tiny_port} --driver
...
realPagesScore=$(cat ./webdriver_score--DO-NOT-FAKE-THIS-OR-YOU-WILL-GET-SCORE-DEDUCTION.txt)
echo "Real Pages: ${realPagesScore} / ${MAX_REAL_PAGES}"
rm ./webdriver_score--DO-NOT-FAKE-THIS-OR-YOU-WILL-GET-SCORE-DEDUCTION.txt

# Emit the total score
# Speed test share points with caching test.
totalScore=`expr ${basicScore} + ${cacheScore} + ${concurrencyScore} + ${realPagesScore}`
maxScore=`expr ${MAX_BASIC} + ${MAX_CACHE} + ${MAX_CONCURRENCY} + ${MAX_REAL_PAGES}`
echo ""
echo "totalScore = ${totalScore} / ${maxScore}"

echo ""
echo "{ \"scores\": {\"Basic\":${basicScore}, \"Concurrency\":${concurrencyScore}, \"Caching\":${cacheScore}, \"Real Pages\":${realPagesScore}},\"scoreboard\": [${totalScore}, ${basicScore}, ${concurrencyScore}, ${cacheScore}, ${realPagesScore}]}"
