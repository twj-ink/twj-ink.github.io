...
void eval_perf(unsigned int s, unsigned int E, unsigned int b)
{
    ...
    for (i=0; i<func_counter; i++) {
        ...
        // Example matrices A and B for validation
        int A[N][M];  // Original matrix
        int B[M][N];  // Transposed matrix
        int B_[M][N];
        initMatrix(M, N, A, B);
        initMatrix(M, N, A, B_);
        correctTrans(M, N, A, B_);
        // Call the transpose function (assuming it fills B with the transpose of A)
        func_list[i].func_ptr(M, N, A, B);

        // Element-wise correctness check
        int correct = 1;  // Assume correct initially
        for (int row = 0; row < M; row++) {
            for (int col = 0; col < N; col++) {
                if (B_[row][col] != B[row][col]) {
                    correct = 0;  // Mark as incorrect
                    break;
                }
            }
            if (!correct) break;
        }
        func_list[i].correct=correct;
