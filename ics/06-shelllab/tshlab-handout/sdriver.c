...
int main(int argc, char **argv)
{
    ...
    int total_score;           /* Total score across all traces */
    ...
    /* Evaluate all trace files */
        ...
        printf("Score: %d/%d\n", total_score, (num_tracefiles < 7 ? num_tracefiles * 4 : 7 * 4 + (num_tracefiles - 7) * 3));
        ...
        if (autograded) {
            int first = 1;
            sprintf(autoresult, "{\"scores\": {\"Correctness\": %d}, \"scoreboard\": [%d,", total_score, total_score);
            for (i = 0; i < num_tracefiles; i++) {
                if (first) {
                    first = 0;
