...
while ($line = <INFILE>) {
    ...
    # Notice that we're ready to read the puzzle scoresz
    if ($line =~ /^Score/) {
	$inpuzzles = 1;
	next;
    }
...
use POSIX qw(ceil);
$final_tpoints = ceil($tpoints * 100 / $trating);
...
# 
if ($autograded) {
    $autoresult = "{ \"scores\": {\"Correctness\":$final_tpoints}, \"scoreboard\": [$final_tpoints, $tops";
    foreach $name (sort {$puzzle_number{$a} <=> $puzzle_number{$b}} 
                   keys %puzzle_number) {
